...
typedef struct nfilosofos{      //Estrutura de dados dos filósofos
        ...
        int contador;           //armazena a quantidade de vezes que cada um comeu
}NFilosofos;
...
void main (){
    ...
    pthread_t *thFilo;          //vetor de threads dos filosofos
        ...
        {
           printf("\nFilosofo: %d   comeu: %dx", i,vetorFilo[i].contador);
        }
...
void comer(void *F){
    NFilosofos *Filo = (NFilosofos*) F;
    int tempo;
    tempo=(rand() % 5+1);       //tempo para comer

    sem_wait(&(macarrao));      //espera o semaforo abrir para comer macarrao
    pthread_mutex_lock(&(m));   //bloqueia a seção critica do macarrao
    
    if(qMacarrao <=0){          //condição para verificar se há macarrão, caso não tenha, termina as threads
     //   printf("\nFilosofo %d foi tentar COMER %d, mas o Macarrao acabou",Filo->id,tempo);
    ...
    qMacarrao = qMacarrao- tempo;   // decrementa do macarrão
    Filo->contador=Filo->contador+1;
    usleep(tempo*100000);       //dorme por um tempo estipulado anteriormente
    if(qMacarrao<=0){           //para não ter macarrão negativo
        qMacarrao=0;
    }
    //printf("\nFilosofo %d COMEU %d, Macarrao restante %d",Filo->id,tempo,qMacarrao);
    pthread_mutex_unlock(&(m)); //desbloqueia o macarrao
    sem_post(&(macarrao));      //libera o semaforo 
